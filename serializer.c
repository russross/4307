...
uint16_t unpack_uint16(struct serializer *ss) {
    assert(ss->cursor + 2 <= ss->size);
    uint16_t partial = ss->base[ss->cursor++];
...
uint32_t unpack_uint24(struct serializer *ss) {
    assert(ss->cursor + 3 <= ss->size);
    uint32_t partial = ss->base[ss->cursor++];
    partial = ss->base[ss->cursor++];
    return (partial << 8) | ss->base[ss->cursor++];
}
...
uint64_t unpack_uint48(struct serializer *ss) {
    assert(ss->cursor + 6 <= ss->size);
    uint64_t partial = ss->base[ss->cursor++];
    partial = (partial << 8) | ss->base[ss->cursor++];
    partial = (partial << 8) | ss->base[ss->cursor++];
    partial = (partial << 8) | ss->base[ss->cursor++];
    partial = (partial << 8) | ss->base[ss->cursor++];
